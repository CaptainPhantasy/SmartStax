    ...
    return (
      <ScrollView contentInsetAdjustmentBehavior="automatic" className="flex-1 bg-white dark:bg-gray-900" style={{ paddingTop: insets.top }}>
        <View className="px-6 py-8 items-center">
          ...
          <Text className="text-gray-900 dark:text-gray-100 font-semibold text-lg mt-4 mb-2">No Stack Selected</Text>
          <Text className="text-gray-600 dark:text-gray-400 text-center mb-6">Please go back and select a technology stack to see your readiness checklist.</Text>
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" className="flex-1 bg-white dark:bg-gray-900" style={{ paddingTop: insets.top }}>
      <View className="px-6 py-8">
        ...
        <View className="mb-8">
          <View className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-5 mb-6">
            <Text className="text-blue-900 dark:text-blue-100 font-bold text-xl mb-2">{selectedStack.title}</Text>
            <Text className="text-blue-800 dark:text-blue-200 text-base leading-6 mb-3">{selectedStack.summary}</Text>
            <View className="flex-row items-center gap-2">
              <Ionicons name="cash-outline" size={16} color="#2563eb" />
              <Text className="text-blue-700 dark:text-blue-300 font-semibold">${selectedStack.est_monthly}/month estimated cost</Text>
            </View>
          </View>

          <View className="flex-row items-center justify-between mb-4">
            <Text className="text-gray-900 dark:text-gray-100 font-bold text-2xl">Readiness Checklist</Text>
            <View className="bg-green-100 dark:bg-green-900/30 px-3 py-1 rounded-full">
              <Text className="text-green-800 dark:text-green-300 font-semibold text-sm">{progress.percentage}% Ready</Text>
            </View>
          </View>
        ...
        <View className="gap-4 mb-8">
          {readinessItems.map((item) => (
            <View key={item.id} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
              <View className="flex-row items-start gap-4">
                <View className="flex-row items-center gap-3 flex-1">
                  ...
                  <View className="flex-1" style={{ minWidth: 0 }}>
                    <Text className="text-gray-900 dark:text-gray-100 font-semibold text-base mb-1" numberOfLines={2} ellipsizeMode="tail">
                      {item.title}
                    </Text>
                    <Text className="text-gray-600 dark:text-gray-400 text-sm leading-5" numberOfLines={3} ellipsizeMode="tail">
                      {item.description}
                    </Text>
                  </View>
                </View>
        ...
        <View className="gap-4">
          ...
          > 
            <Text className="text-gray-900 dark:text-gray-100 font-semibold text-base">Change Stack</Text>
          </Pressable>
